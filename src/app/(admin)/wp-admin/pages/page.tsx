    ...
    return (
        <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-4 mb-6">
                <h1 className="text-2xl font-normal text-[#1d2327]">Páginas</h1>
                <Link href="/wp-admin/pages/new">
                ...
                <table className="w-full text-left text-sm text-[#50575e]">
                    <thead className="border-b border-[#c3c4c7] font-semibold text-[#2c3338]">
                        <tr>
                            ...
                            <th className="px-4 py-2 font-medium">Padre</th>
                    ...
                    <tbody className="divide-y divide-[#f0f0f1]">
                        {pages.length === 0 ? (
                            ...
                            pages.map((page) => (
                                <tr key={page.id} className="hover:bg-[#f6f7f7] group">
                                    <td className="px-4 py-3 align-top">
                                        <Link href={`/wp-admin/pages/${page.id}`} className="font-semibold text-[#2271b1] hover:text-[#135e96] block text-base">
                                            {page.title || "(Sin título)"}
                                        </Link>
                                        <div className="invisible group-hover:visible flex gap-2 text-[13px] mt-1 text-[#2271b1]">
                                            <Link href={`/wp-admin/pages/${page.id}`} className="hover:underline">Editar</Link>
                                            <span className="text-[#c3c4c7]">|</span>
                                            <button className="text-[#d63638] hover:underline">Papelera</button>
                                            <span className="text-[#c3c4c7]">|</span>
                                            <Link href={`/${page.slug}`} className="hover:underline" target="_blank">Ver</Link>
                                        </div>
                                    </td>
                                    <td className="px-4 py-3 align-top">
                                        <Link href={`/wp-admin/users/${page.authorId}`} className="text-[#2271b1] hover:underline">
                                    ...
                                    <td className="px-4 py-3 align-top">
                                        {page.status === "PUBLISHED" ? "Publicada" : "Última modificación"}<br />
                                        {format(page.updatedAt, "dd/MM/yyyy 'a las' HH:mm", { locale: es })}
                                    </td>
                                </tr>
                            ))
                        )}
